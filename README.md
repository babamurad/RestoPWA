...
RestoPWA — это современная платформа для доставки еды, построенная по принципам **Domain-Driven Design (DDD)** с использованием **Laravel 12**, **Livewire 3** и **Tailwind CSS**. Платформа поддерживает мультитенантность (один инстанс обслуживает несколько ресторанов) и работает как Progressive Web App (PWA) с поддержкой офлайн-режима.
