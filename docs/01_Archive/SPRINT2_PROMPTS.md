...
- [x] S2-1: `php artisan test` — все тесты зелёные
- [x] S2-1: `./vendor/bin/pint --test` — нет нарушений стиля
- [x] S2-1: `npm run test` — Vitest зелёный
- [x] S2-2: `AccessControlTest.php` создан и проходит
- [x] S2-3: `idempotency_key` миграция + контроллер + клиент
- [x] S2-4: единый формат API ошибок
- [x] S2-5: checkout conflict UX с блокировкой submit
- [x] S2-6: CHANGELOG + DoD + ТЗ обновлены

🚀 Что осталось впереди (Дорожная карта)
Сейчас базовая (критическая) функциональность для запуска MVP готова. Следующие приоритеты:

1. Sprint 3 (текущий/следующий приоритет): E2E smoke-тестирование (сквозной тест сценариев) + багфикс. Дополнительно — опциональные signed URL для отслеживания гостевых заказов.
2. Sprint 4 (Этап P2): Real-time tracking — добавление WebSocket (Reverb) для онлайн-наблюдения за статусом заказа.
3. Sprint 5: Push-уведомления (инфраструктура уже частично под капотом, предстоит сделать UI выбора).
4. Sprint 6: Интеграция реальных платежей и доработка Discovery (рейтинги/отзывы/скидки).
