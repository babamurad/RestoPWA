<template>
  <div class="px-4 py-6 max-w-lg mx-auto md:max-w-4xl lg:max-w-6xl pb-24 font-inter">
    ...
    <div class="flex items-center gap-3 mb-8">
      <router-link :to="cartStore.vendorSlug ? `/restaurants/${cartStore.vendorSlug}` : '/'" class="w-10 h-10 rounded-xl bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700/50 flex items-center justify-center text-slate-700 dark:text-slate-300 hover:text-orange-400 hover:border-orange-500/30 transition-all active:scale-95">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
        </svg>
      </router-link>
      <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100 font-outfit tracking-wide">Моя корзина</h2>
    </div>

    <!-- Empty State -->
    <div v-if="cartStore.isEmpty" class="flex flex-col items-center justify-center py-16 text-center select-none animate-pulse">
      <div class="w-24 h-24 rounded-full bg-white dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700/30 flex items-center justify-center mb-6 shadow-xl relative overflow-hidden group">
        <div class="absolute inset-0 bg-gradient-to-tr from-orange-500/10 to-amber-500/10 blur-xl opacity-100"></div>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10 text-slate-500 dark:text-slate-500">
          <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
        </svg>
      </div>
      <h3 class="text-lg font-bold text-slate-800 dark:text-slate-200 mb-2 font-outfit">Корзина пуста</h3>
      <p class="text-sm text-slate-600 dark:text-slate-400 max-w-xs mb-8 font-medium">
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <div class="bg-white dark:bg-slate-800/30 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-4 flex items-center justify-between select-none">
          <div class="flex items-center gap-3">
            <span class="text-2xl">🍔</span>
            <div>
              <span class="text-xs text-slate-500 dark:text-slate-400 block font-black uppercase tracking-wider">Заказ из ресторана</span>
              <h4 class="text-sm font-black text-slate-900 dark:text-slate-100 group-hover:text-orange-400 hover:underline cursor-pointer" @click="$router.push(`/restaurants/${cartStore.vendorSlug}`)">
        ...
        <div class="space-y-3 select-none">
          <div 
            ...
            :key="item.id" 
            class="bg-white dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800/50 rounded-2xl p-4 flex gap-4 items-center justify-between shadow-md hover:border-slate-300 dark:hover:border-slate-700/40 transition-all duration-300 animate-fade-in"
          >
            <div class="flex items-center gap-3 flex-1 min-w-0">
              <img 
                :src="item.image" 
                :alt="item.name" 
                class="w-16 h-16 rounded-xl object-cover border border-slate-200 dark:border-slate-800 shadow-inner"
                @error="handleImageError"
              />
              <div class="min-w-0 flex-1">
                <h5 class="text-sm font-bold text-slate-900 dark:text-slate-100 truncate">{{ item.name }}</h5>
                <p v-if="item.weight" class="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Вес: {{ item.weight }} г</p>
                <p class="text-xs text-slate-600 dark:text-slate-400 mt-1 font-semibold">{{ item.price }} TMT x {{ item.quantity }}</p>
                <div class="text-xs font-black text-orange-400 mt-1.5">{{ (item.price * item.quantity).toFixed(0) }} TMT</div>
              </div>
            </div>

            <!-- Item Quantity Buttons -->
            <div class="flex items-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-1 shadow-inner flex-shrink-0">
              <button 
                @click="decreaseQty(item)" 
                class="w-7 h-7 rounded-lg bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center justify-center text-slate-700 dark:text-slate-300 font-bold active:scale-90 transition-all"
              >
                -
              </button>
              <span class="w-8 text-center text-xs font-black text-slate-800 dark:text-slate-200">{{ item.quantity }}</span>
              <button 
      ...
      <div class="lg:col-span-1 select-none">
        <div class="bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-6 shadow-xl sticky top-6 space-y-6">
          <h4 class="text-base font-extrabold text-slate-900 dark:text-slate-100 font-outfit tracking-wide">Детали заказа</h4>
          
          <div class="space-y-3.5 text-sm text-slate-600 dark:text-slate-400 font-semibold border-b border-slate-300 dark:border-slate-700/30 pb-5">
            ...
            <div class="flex justify-between">
              <span>Доставка</span>
              <span class="text-slate-800 dark:text-slate-200">{{ cartStore.deliveryFee === 0 ? 'Бесплатно' : cartStore.deliveryFee + ' TMT' }}</span>
            </div>
          </div>
          ...
          <div class="flex justify-between items-baseline">
            <span class="text-sm font-bold text-slate-600 dark:text-slate-400 font-outfit">Итого</span>
          ...
          <button 
            ...
            class="w-full py-4 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 disabled:from-white dark:disabled:from-slate-800 disabled:to-slate-50 dark:disabled:to-slate-800 disabled:text-slate-500 dark:disabled:text-slate-500 disabled:cursor-not-allowed text-white font-extrabold text-sm rounded-2xl shadow-lg shadow-orange-500/10 hover:shadow-orange-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
          >
            Оформить заказ
