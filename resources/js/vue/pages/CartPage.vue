<template>
  <div class="px-4 py-6 max-w-lg mx-auto md:max-w-4xl lg:max-w-6xl pb-24 font-inter">
    ...
    <div class="flex items-center gap-3 mb-8">
      <router-link :to="cartStore.vendorSlug ? `/restaurants/${cartStore.vendorSlug}` : '/'" class="w-10 h-10 rounded-xl dark:bg-slate-800 border dark:border-slate-700/50 flex items-center justify-center dark:text-slate-300 hover:text-orange-400 hover:border-orange-500/30 transition-all active:scale-95 bg-white border-slate-300 text-slate-700">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
        </svg>
      </router-link>
      <h2 class="text-xl font-bold dark:text-slate-100 font-outfit tracking-wide text-slate-900">Моя корзина</h2>
    </div>

    <!-- Empty State -->
    <div v-if="cartStore.isEmpty" class="flex flex-col items-center justify-center py-16 text-center select-none animate-pulse">
      <div class="w-24 h-24 rounded-full dark:bg-slate-800/50 border dark:border-slate-700/30 flex items-center justify-center mb-6 shadow-xl relative overflow-hidden group bg-white border-slate-300">
        <div class="absolute inset-0 bg-gradient-to-tr from-orange-500/10 to-amber-500/10 blur-xl opacity-100"></div>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10 dark:text-slate-500 text-slate-500">
          <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
        </svg>
      </div>
      <h3 class="text-lg font-bold dark:text-slate-200 mb-2 font-outfit text-slate-800">Корзина пуста</h3>
      <p class="text-sm dark:text-slate-400 max-w-xs mb-8 font-medium text-slate-600">
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <div class="dark:bg-slate-800/30 border dark:border-slate-800/80 rounded-2xl p-4 flex items-center justify-between select-none bg-white border-slate-200">
          <div class="flex items-center gap-3">
            <span class="text-2xl">🍔</span>
            <div>
              <span class="text-xs dark:text-slate-400 block font-black uppercase tracking-wider text-slate-600">Заказ из ресторана</span>
              <h4 class="text-sm font-black dark:text-slate-100 group-hover:text-orange-400 hover:underline cursor-pointer text-slate-900" @click="$router.push(`/restaurants/${cartStore.vendorSlug}`)">
        ...
        <div class="space-y-3 select-none">
          <div 
            ...
            :key="item.id" 
            class="dark:bg-slate-800/40 border dark:border-slate-800/50 rounded-2xl p-4 flex gap-4 items-center justify-between shadow-md dark:hover:border-slate-700/40 transition-all duration-300 animate-fade-in bg-white border-slate-200 hover:border-slate-300"
          >
            <div class="flex items-center gap-3 flex-1 min-w-0">
              <img 
                :src="item.image" 
                :alt="item.name" 
                class="w-16 h-16 rounded-xl object-cover border dark:border-slate-800 shadow-inner border-slate-200"
                @error="handleImageError"
              />
              <div class="min-w-0 flex-1">
                <h5 class="text-sm font-bold dark:text-slate-100 truncate text-slate-900">{{ item.name }}</h5>
                <p v-if="item.weight" class="text-[10px] dark:text-slate-400 mt-0.5 text-slate-600">Вес: {{ item.weight }} г</p>
                <p class="text-xs dark:text-slate-400 mt-1 font-semibold text-slate-600">{{ item.price }} TMT x {{ item.quantity }}</p>
                <div class="text-xs font-black text-orange-400 mt-1.5">{{ (item.price * item.quantity).toFixed(0) }} TMT</div>
              </div>
            </div>

            <!-- Item Quantity Buttons -->
            <div class="flex items-center dark:bg-slate-900 border dark:border-slate-800 rounded-xl p-1 shadow-inner flex-shrink-0 bg-slate-50 border-slate-200">
              <button 
                @click="decreaseQty(item)" 
                class="w-7 h-7 rounded-lg dark:bg-slate-800 dark:hover:bg-slate-700 flex items-center justify-center dark:text-slate-300 font-bold active:scale-90 transition-all bg-white hover:bg-slate-100 text-slate-700"
              >
                -
              </button>
              <span class="w-8 text-center text-xs font-black dark:text-slate-200 text-slate-800">{{ item.quantity }}</span>
              <button 
      ...
      <div class="lg:col-span-1 select-none">
        <div class="dark:bg-slate-800 border dark:border-slate-800/80 rounded-3xl p-6 shadow-xl sticky top-6 space-y-6 bg-white border-slate-200">
          <h4 class="text-base font-extrabold dark:text-slate-100 font-outfit tracking-wide text-slate-900">Детали заказа</h4>
          
          <div class="space-y-3.5 text-sm dark:text-slate-400 font-semibold border-b dark:border-slate-700/30 pb-5 text-slate-600 border-slate-300">
            ...
            <div class="flex justify-between">
              <span>Доставка</span>
              <span class="dark:text-slate-200 text-slate-800">{{ cartStore.deliveryFee === 0 ? 'Бесплатно' : cartStore.deliveryFee + ' TMT' }}</span>
            </div>
          </div>
          ...
          <div class="flex justify-between items-baseline">
            <span class="text-sm font-bold dark:text-slate-400 font-outfit text-slate-600">Итого</span>
          ...
          <button 
            ...
            class="w-full py-4 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 dark:disabled:from-slate-800 dark:disabled:to-slate-800 dark:disabled:text-slate-500 disabled:cursor-not-allowed text-white font-extrabold text-sm rounded-2xl shadow-lg shadow-orange-500/10 hover:shadow-orange-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:from-slate-200 disabled:to-slate-200 disabled:text-slate-500"
          >
            Оформить заказ
...
const clearCart = () => {
  if (window.Swal) {
    window.Swal.fire({
      ...
      color: '#f8fafc',
      reverseButtons: true,
      customClass: { popup: 'rounded-3xl border dark:border-slate-800 shadow-2xl border-slate-200' }
    }).then((result) => {
