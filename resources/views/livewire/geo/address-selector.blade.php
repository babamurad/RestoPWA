<div x-data="{
        ...
        init() {
            this.$watch('isLocalModalOpen', value => {
                if (value === true) {
                    // Ждем завершения всех анимаций Alpine
        ...
        initMap() {
            ...
            ymaps.ready(() => {
                ...
                // Если карта уже есть, просто обновляем её состояние
                ...
                try {
                    ...
                    console.log('Creating new map instance at:', lat, lon);

                    // Создаём карту без controls — zoomControl в скрытом/анимирующем
                    // контейнере формирует невалидные pixel-shapes → ошибка setGeometry.
                    ...
                    // Скрываем спиннер сразу после создания карты — контейнер карты
                    // (yandex-map-v2) виден всегда (спиннер — просто overlay поверх него),
                    // поэтому размеры корректны и сбросить isMapLoading здесь безопасно.
                    // Это гарантирует, что спиннер уберётся ДАЖЕ если что-то ниже упадёт.
                    this.mapInitialized = true;
                    this.isMapLoading = false;

                    // Controls и Placemark добавляем на первый boundschange —
                    // к этому моменту карта точно знает свои пиксельные размеры.
                    this.mapInstance.events.once('boundschange', () => {
                        ...
                        try {
                            ...
                            this.markerInstance.events.add('dragend', () => {
                                ...
                                $wire.setLocation(coords[0], coords[1]);
                            });
                        ...
                        // Следим за изменениями координат из Livewire
