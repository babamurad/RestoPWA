...
<body class="bg-slate-950 text-slate-100 antialiased">
    <div x-data="orderTracker('{{ $orderId }}', '{{ $signedApiUrl }}')" x-init="init()" class="min-h-screen bg-slate-950">
        ...
        <header class="sticky top-0 z-40 bg-slate-900/60 backdrop-blur-xl border-b border-slate-800/40 transition-all">
            <div class="max-w-lg mx-auto md:max-w-4xl lg:max-w-6xl px-4 h-14 md:h-16 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <a href="{{ route('orders.index') }}" class="flex items-center justify-center w-10 h-10 -ml-2 rounded-full hover:bg-slate-800/40 transition-all">
                    ...
                    <h1 class="text-lg md:text-xl font-bold text-slate-100 truncate mt-0.5">Заказ #{{ substr($orderId, 0, 8) }}</h1>
                </div>
                <span 
                            ...
                            @foreach(['pending', 'confirmed', 'preparing', 'ready', 'delivering', 'completed'] as $index => $status)
                                <div class="flex items-center gap-4 group">
                                    <div 
                                        ...
                                        :class="statusOrder.indexOf('{{ $status }}') <= statusOrder.indexOf(currentStatus) ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20' : 'bg-slate-800/50 text-slate-500 border border-slate-700/30'"
                                    >
                                    ...
                                    <div class="min-w-0">
                                        <p class="text-xs font-bold transition-colors"
                                           :class="statusOrder.indexOf('{{ $status }}') <= statusOrder.indexOf(currentStatus) ? 'text-slate-100' : 'text-slate-500'"
                                        >
                                            @switch($status)
                                        ...
                                        <p class="text-[10px] text-slate-400 font-medium truncate" x-show="statusOrder.indexOf('{{ $status }}') <= statusOrder.indexOf(currentStatus)">
                ...
                <div class="mt-6 md:mt-0 space-y-6">
                    ...
                    <div class="bg-slate-900/60 rounded-3xl p-6 shadow-xl border border-slate-800/40">
                        ...
                        @if(!empty($addr['address']))
                            <p class="text-sm font-medium text-slate-300 leading-relaxed">{{ $addr['address'] }}</p>
                        @endif
                        @if(!empty($addr['manual_address']))
                            <p class="text-sm font-medium text-slate-300 leading-relaxed">{{ $addr['manual_address'] }}</p>
                        @endif
                        @if(!empty($addr['landmark']) || !empty($addr['entrance']) || !empty($addr['floor']) || !empty($addr['apartment']))
                            <div class="flex flex-wrap gap-1.5 mt-2">
                                ...
                                @if(!empty($addr['entrance']))<span class="inline-flex items-center px-2 py-0.5 bg-slate-800 rounded-lg text-xs font-medium text-slate-300">п. {{ $addr['entrance'] }}</span>@endif
                                @if(!empty($addr['floor']))<span class="inline-flex items-center px-2 py-0.5 bg-slate-800 rounded-lg text-xs font-medium text-slate-300">эт. {{ $addr['floor'] }}</span>@endif
                                @if(!empty($addr['apartment']))<span class="inline-flex items-center px-2 py-0.5 bg-slate-800 rounded-lg text-xs font-medium text-slate-300">кв. {{ $addr['apartment'] }}</span>@endif
                            </div>
                        @endif
                        @if(!empty($addr['courier_comment']))
                            <p class="text-xs text-slate-400 italic mt-2">«{{ $addr['courier_comment'] }}»</p>
                        @endif
                        @if(!empty($addr['lat']) && !empty($addr['lon']))
                            <div class="flex gap-2 mt-3 pt-3 border-t border-slate-800/40">
                                <a href="https://www.google.com/maps?q={{ $addr['lat'] }},{{ $addr['lon'] }}" target="_blank" rel="noopener noreferrer" class="flex-1 py-2 bg-slate-800/50 text-slate-300 text-xs font-bold rounded-xl hover:bg-slate-700/50 border border-slate-700/30 transition-all text-center flex items-center justify-center gap-1.5">
                                ...
                                <button onclick="navigator.clipboard.writeText('{{ $addr['lat'] }}, {{ $addr['lon'] }}')" class="flex-1 py-2 bg-slate-800/50 text-slate-300 text-xs font-bold rounded-xl hover:bg-slate-700/50 border border-slate-700/30 transition-all text-center flex items-center justify-center gap-1.5">
                            ...
                            @foreach($order->items as $item)
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-xs font-bold text-slate-100 truncate leading-tight">{{ $item['name'] ?? 'Товар' }}</p>
                                        <p class="text-[10px] text-slate-400 font-medium">Кол-во: {{ $item['quantity'] ?? 1 }}</p>
                                    </div>
                                    <p class="text-xs font-bold text-slate-100">{{ number_format(($item['price'] ?? 0) * ($item['quantity'] ?? 1), 0, '.', ' ') }} ₽</p>
                                </div>
                            @endforeach
                        </div>
