    ...
    <div class="bg-white rounded-lg shadow p-6 space-y-6">
        ...
        <div>
            ...
            <input type="number" name="price" value="{{ old('price', $product->price) }}" required step="0.01" min="0"
                   class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500 @error('price') border-red-500 @enderror">
            @error('price')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <select name="restaurant_id" required
                ...
                <option value="{{ $r->id }}" {{ old('restaurant_id', $product->restaurant_id) == $r->id ? 'selected' : '' }}>{{ $r->name }}</option>
                @endforeach
            </select>
            @error('restaurant_id')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        ...
        <div>
            ...
            <select name="category_id" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500">
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500">{{ old('description', $product->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            <input type="text" name="image" value="{{ old('image', $product->image) }}"
                   class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500" placeholder="https://...">
        </div>
        ...
        <div>
            <label class="flex items-center">
                ...
                <span class="ml-2 text-sm text-gray-700">Доступен для заказа</span>
            </label>
        </div>

        <div class="flex justify-end gap-3 pt-4 border-t">
            <a href="{{ route('admin.products.index') }}" class="px-4 py-2 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200">Отмена</a>
