...
$__newAttributes = [];
$__propNames = \Illuminate\View\ComponentAttributeBag::extractPropNames((['showSearch' => true, 'showProfile' => true]));
...
foreach (array_filter((['showSearch' => true, 'showProfile' => true]), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
}
...
<header class="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-sm">
    <div class="max-w-lg mx-auto md:max-w-4xl lg:max-w-6xl xl:max-w-7xl flex items-center gap-2 sm:gap-3 px-3 sm:px-4 h-14 md:h-16">
        <a href="<?php echo e(route('home')); ?>" class="flex items-center gap-1.5 sm:gap-2 shrink-0 hover:opacity-80 transition-opacity">
        ...
        <?php if (! $__env->hasRenderedOnce('1a9fd55e-c7bf-4e91-864a-4fad59485fd1')): $__env->markAsRenderedOnce('1a9fd55e-c7bf-4e91-864a-4fad59485fd1'); ?>
        <script>
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($showSearch): ?>
            <div class="flex-1 max-w-xl mx-auto hidden md:block">
                <form action="<?php echo e(request()->routeIs('home') ? route('home') : route('restaurants.index')); ?>" 
                      method="GET" 
                      class="relative"
                      x-data="liveSearch('<?php echo e(request('search')); ?>')"
                      @submit.prevent>
                    <input type="text" 
                           name="search"
                           x-model="search"
        ...
        <div class="flex items-center gap-2">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($showProfile): ?>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->guard()->check()): ?>
                    ...
                    <?php elseif(auth()->user()->isRestaurateur()): ?>
                        <a href="<?php echo e(route('vendor.orders.kanban')); ?>" class="hidden sm:flex items-center justify-center w-10 h-10 rounded-full hover:bg-orange-50 transition-colors" title="Панель ресторана">
                    ...
                    <a href="<?php echo e(route('profile.edit')); ?>" class="flex items-center gap-3 px-3 py-1.5 rounded-full hover:bg-gray-50 border border-transparent hover:border-gray-100 transition-all">
                        <div class="hidden md:block text-right">
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none mb-1">Профиль</p>
                            <p class="text-sm font-bold text-gray-900 leading-none"><?php echo e(auth()->user()->name); ?></p>
                        </div>
                ...
                <?php else: ?>
                    <a href="<?php echo e(route('login')); ?>" class="px-4 sm:px-6 py-1.5 sm:py-2 shrink-0 bg-gray-900 text-white text-xs sm:text-sm font-bold rounded-xl hover:bg-black transition-all">
                        Войти
                    </a>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
...
<div class="flex items-center gap-2 px-4 py-2 bg-orange-50 border-b border-orange-100">
    ...
    <span class="text-sm text-orange-700 truncate"><?php echo e($address ?? 'ул. Ленина, 15, кв. 42'); ?></span>
</div><?php /**PATH C:\OSPanel\domains\RestoPWA\resources\views/components/header.blade.php ENDPATH**/ ?>